                  ...
                  <td className="px-4 py-2.5 text-ink-500 whitespace-nowrap">
                    {item.posted_at ? (
                      ...
                      <span className="text-ink-600">
                        {new Date(item.scheduled_at).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}
                      </span>
                    ) : (
                      '—'
                    )}
        ...
        <div className="flex items-start justify-between mb-3 gap-4">
          ...
          <div className="flex gap-2 flex-shrink-0 flex-wrap justify-end">
            {campaign.status === 'active' && (
              <button onClick={() => onToggle(campaign.id, 'paused')} className="btn-ghost py-1.5 text-xs">
                <Pause size={13} /> Pause
              </button>
            )}
...
function BotFormInner({ initial, isEdit, onClose, onSaved }) {
  ...
  const validate = () => {
    ...
    const endHour = toInt(form.post_end_hour, 20)
    const minDelay = toInt(form.min_delay_seconds, 480)
    const maxDelay = toInt(form.max_delay_seconds, 900)
    const sessionCap = toInt(form.session_cap, 8)
    const breakMin = toInt(form.session_break_min, 120)
    const breakMax = toInt(form.session_break_max, 180)

    if (maxPosts < 1) return 'Max posts per day must be at least 1'
    if (startHour < 0 || startHour > 23) return 'Start hour must be between 0 and 23'
    if (endHour < 1 || endHour > 24) return 'End hour must be between 1 and 24'
    if (endHour <= startHour) return 'End hour must be greater than start hour'
    if (minDelay < 0) return 'Minimum delay cannot be negative'
    if (maxDelay < minDelay) return 'Maximum delay must be greater than or equal to minimum delay'
    if (sessionCap < 1) return 'Session cap must be at least 1'
    if (breakMin < 0) return 'Minimum break cannot be negative'
    if (breakMax < breakMin) return 'Maximum break must be greater than or equal to minimum break'
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        min_delay_seconds: toInt(form.min_delay_seconds, 480),
        max_delay_seconds: toInt(form.max_delay_seconds, 900),
        session_cap: toInt(form.session_cap, 8),
        session_break_min: toInt(form.session_break_min, 120),
        session_break_max: toInt(form.session_break_max, 180),
        session_file: isEdit
  ...
  return (
    <div className="space-y-4">
        ...
        {[['account', 'Account'], ['settings', 'Bot Settings']].map(([v, l]) => (
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors ${
              tab === v ? 'bg-ink-700 text-ink-100' : 'text-ink-500 hover:text-ink-300'
            }`}
          >
            {l}
          </button>
      ...
      {tab === 'settings' && (
        <div className="space-y-4">
          <div className="bg-ink-700/50 border border-ink-600/50 rounded-xl p-3 text-xs text-ink-300 leading-relaxed">
            These settings control per-bot pacing and posting limits on the server.
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <NumberField
              ...
              onChange={v => set('max_posts_per_day', v)}
              min={1}
              hint="Maximum number of posts this bot can make in one day."
            />
            <NumberField
              label="Session Cap"
              value={form.session_cap}
              onChange={v => set('session_cap', v)}
              min={1}
              hint="How many posts the bot can do in one active session."
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <NumberField
              label="Posting Start Hour"
              value={form.post_start_hour}
              onChange={v => set('post_start_hour', v)}
              min={0}
              max={23}
              hint="24-hour format. Example: 9 = 9:00 AM."
            />
            <NumberField
              label="Posting End Hour"
              value={form.post_end_hour}
              onChange={v => set('post_end_hour', v)}
              min={1}
              max={24}
              hint="24-hour format. Example: 20 = 8:00 PM."
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <NumberField
              ...
              onChange={v => set('min_delay_seconds', v)}
              min={0}
              hint="Minimum wait time between posts."
            />
            <NumberField
              label="Max Delay (seconds)"
              value={form.max_delay_seconds}
              onChange={v => set('max_delay_seconds', v)}
              min={0}
              hint="Maximum wait time between posts."
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <NumberField
              label="Session Break Min (seconds)"
              value={form.session_break_min}
              onChange={v => set('session_break_min', v)}
              min={0}
              hint="Minimum break after a posting session ends."
            />
            <NumberField
              label="Session Break Max (seconds)"
              value={form.session_break_max}
              onChange={v => set('session_break_max', v)}
              min={0}
              hint="Maximum break after a posting session ends."
            />
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button type="button" onClick={onClose} className="btn-ghost py-2.5 px-5">
          Cancel
        </button>
      </div>
    </div>
      ...
      <div>
        <div className="bg-ink-950 rounded-xl border border-ink-700 h-80 overflow-y-auto p-3 font-mono text-xs leading-relaxed">
          {lines.length === 0 ? (
            ...
            lines.map((line, i) => (
              <div key={i} className={`${color(line)} py-0.5`}>
                {line}
              </div>
            ))
          )}
    ...
    <Modal open={open} onClose={onClose} title={`Start Bot — ${bot.name}`} size="sm">
      <div className="space-y-4">
              ...
              return (
                ...
                >
                  ...
                  <p className="text-xs text-ink-500 mt-0.5">
                    {pending} pending · {posted} posted · {items.length} total
                  </p>
                </div>
              )
            })}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={onClose} className="btn-ghost py-2.5 px-4">
            Cancel
          </button>
        </div>
      </div>
...
function CampaignQueueModal({ open, bot, campaigns, onClose }) {
  ...
  const saveQueue = async () => {
    setSaving(true)
    try {
      await setBotQueue(bot.id, queue)
    } catch {}
    setSaving(false)
    onClose()
  }
                ...
                return (
                  <div key={cid} className="flex items-center gap-2 bg-ink-800 border border-ink-700 rounded-xl px-3 py-2">
                    ...
                    <div className="flex items-center gap-1">
                      {i > 0 && (
                        <button onClick={() => moveUp(i)} className="text-ink-600 hover:text-ink-300 p-1 text-xs">
                          ↑
                        </button>
                      )}
...
function BotCard({ bot, campaigns, serverOnline, onEdit, onDelete, onRefresh }) {
  ...
  const doStart = async (botId, campaignId) => {
    setBusy('start')
    try {
      await startBot(botId, campaignId)
    } catch {}
    onRefresh()
    setBusy(null)
  }
    ...
    <>
      <div className={`rounded-2xl border overflow-hidden ${bot.active !== false ? 'border-ink-700 bg-ink-800/30' : 'border-ink-700/30 opacity-50'}`}>
        ...
        <div className="px-4 pb-3">
          {bot.hasSession ? (
            <p className="text-xs text-jade-500 flex items-center gap-1.5">
              <CheckCircle size={10} /> Session saved
            </p>
          ) : (
            <p className="text-xs text-yellow-500 flex items-center gap-1.5">
              <AlertTriangle size={10} /> Login required
            </p>
          )}
        </div>
        ...
        <div className="px-3 pb-3 space-y-2">
            ...
            <button
              ...
              className={`w-full py-2 text-xs rounded-xl border font-medium flex items-center justify-center gap-1.5 transition-all ${
                canLogin && busy !== 'login'
                  ...
                  : 'bg-ink-800/50 border-ink-700/50 text-ink-600 cursor-not-allowed'
              }`}
            >
              {busy === 'login' ? (
                <>
          ...
          {canStop ? (
            ...
            <button
              ...
              className={`w-full py-2 text-xs rounded-xl font-medium flex items-center justify-center gap-1.5 transition-all ${
                canStart && busy !== 'start'
                  ? 'bg-jade-500 text-white hover:bg-jade-400'
                  : 'bg-ink-700 text-ink-600 cursor-not-allowed border border-ink-600'
              }`}
            >
          ...
          <div className="flex gap-1.5">
            ...
            <button onClick={() => onEdit(bot)} className="btn-ghost py-1.5 text-xs px-3">
              <Edit2 size={12} />
            </button>
            <button onClick={() => onDelete(bot.id)} className="btn-ghost py-1.5 text-xs px-3 hover:text-flame-400">
              <Trash2 size={12} />
            </button>
          </div>
        </div>
        ...
        {!serverOnline && (
          <div className="px-3 pb-3 text-center">
            <p className="text-xs text-ink-600">Bot API not reachable right now</p>
          </div>
        )}
      </div>
    ...
    <div className="card overflow-hidden">
      <div className="px-5 py-4 border-b border-ink-700 flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink-500 mt-0.5">
              {bots.length === 0 ? 'Add Facebook accounts to start automating' : `${activeBots.length} bot${activeBots.length !== 1 ? 's' : ''} · ~${activeBots.length * 18} posts/day`}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          <button onClick={refresh} className="btn-ghost py-2 text-xs px-3">
            <RefreshCw size={12} />
          </button>
          <button onClick={() => setFormModal('add')} className="btn-primary py-2 text-xs">
            <Plus size={13} /> Add Bot Account
          </button>
        </div>
      </div>
        ...
        <div className="px-5 py-3 border-b border-ink-700/40 bg-yellow-500/5 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-yellow-400/70 mt-1.5">
              The dashboard is using <code className="text-flame-300">{BOT_API}</code>.
            </p>
          </div>
        </div>
      )}
      ...
      {!loading && bots.length === 0 ? (
        <div className="p-12 text-center">
          ...
          <button onClick={() => setFormModal('add')} className="btn-primary py-2.5 px-6 inline-flex">
            <Plus size={14} /> Add First Bot
          </button>
        </div>
      ) : !loading && bots.length > 0 ? (
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {bots.map(bot => (
            <BotCard key={bot.id} bot={bot} campaigns={campaigns} serverOnline={online} onEdit={b => setFormModal(b)} onDelete={handleDelete} onRefresh={refresh} />
          ))}
        </div>
      ) : null}

      <BotFormModal open={formModal !== null} initial={formModal === 'add' ? null : formModal} onClose={() => setFormModal(null)} onSaved={refresh} />
    </div>
  )
}
...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8 fade-up">
      ...
      <div className="mb-8">
        <BotAccountsSection campaigns={campaigns} />
      </div>

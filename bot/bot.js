...
async function main() {
  ...
  while (true) {
    ...
    const success = await postToGroup(page, item)
...
// ── Post to one group ─────────────────────────────────────
async function postToGroup(page, item) {
  ...
  try {
    ...
    if (photos.length > 0 && propId) {
      ...
      if (cachedPaths.length > 0) {
        await attachPhotos(page, cachedPaths)
...
async function attachPhotos(page, localPaths) {
